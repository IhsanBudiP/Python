...
def backup_switch_config(device_ip, device_username, device_password, backup_dir, device_name, wb, sheet, timeout=60):
    ...
    try:
        ssh_command = [
            ...
            "ssh", f"{device_username}@{device_ip}",
            "show running-config"
        ]
        with open(backup_file, "w") as outfile:
            subprocess.run(ssh_command, stdout=outfile, check=True, universal_newlines=True, timeout=timeout)
        ...
        logger.error(f"Timeout occurred during backup of {device_ip}")
        sheet.append([device_ip, device_name, "Timeout", "", timestamp])
    except subprocess.CalledProcessError as e:
        logger.error(f"Error backing up {device_ip}: {e}")
        sheet.append([device_ip, device_name, f"Error: {e}", "", timestamp]) 
    except OSError as e:
...
def backup_multiple_switches(switch_configs, username, password, base_backup_dir, excel_base_dir): 
    """Backs up multiple aruba devices & routers and generates an Excel report organized by year and month."""
    ...
    for device_ip, backup_dir_name in switch_configs.items():
        backup_dir = os.path.join(base_backup_dir, backup_dir_name) 
        os.makedirs(backup_dir, exist_ok=True)
        backup_switch_config(device_ip, username, password, backup_dir, backup_dir_name, wb, sheet)
    ...
    shutil.copy2(excel_file, backup_file)
        #print(f"Excel report copied to: {backup_file}")
    #except Exception as e:
        #print(f"Error copying Excel report: {e}")
...
if __name__ == "__main__":
    ...
    backup_multiple_switches(switch_configs, username, password, base_backup_dir, excel_base_dir)